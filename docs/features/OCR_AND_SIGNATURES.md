...
1. **For Signature Pages:**
   - Use `EXTRACTION_METHOD=hybrid` + `LLM_PROCESSING_MODE=multimodal` for best results
   - Vision API can read both printed and handwritten text
   - Better context understanding

2. **For General Mixed PDFs:**
   - Use `EXTRACTION_METHOD=ocr_images_only` + `LLM_PROCESSING_MODE=text_llm`
   - OCR extracts text from image pages
   - Combined text sent to text LLM

3. **For Fully Scanned Documents:**
   - Use `EXTRACTION_METHOD=ocr_all` + `LLM_PROCESSING_MODE=text_llm`
   - OCR all pages
   - Send OCR text to text LLM

4. **For Text-Only Documents:**
   - Use `EXTRACTION_METHOD=text_direct`
...
Set in `.env`:
```env
# For signature pages (recommended)
EXTRACTION_METHOD=hybrid
LLM_PROCESSING_MODE=multimodal

# For general mixed PDFs
...
**Possible causes:**
1. Using `text_direct` extraction method (skips image pages)
2. Using `ocr_images_only` method with handwritten signatures (OCR can't read handwriting)
3. Image quality too low
4. Signature page not detected

**Solutions:**
1. Use `LLM_PROCESSING_MODE=multimodal` for signature pages
...
**Solutions:**
1. Use `LLM_PROCESSING_MODE=multimodal` instead of OCR
